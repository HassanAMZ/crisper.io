      ...
      >
        {files.map((f, idx) => (
          <figure
            key={idx}
            className={cn(
              "relative w-32 cursor-pointer overflow-hidden rounded border p-4",
  ...
  },
  {
    ...
    background: (
      <Calendar
        ...
        className="absolute top-10 right-0 origin-top scale-75 rounded border [mask-image:linear-gradient(to_top,transparent_40%,#000_100%)] transition-all duration-300 ease-out group-hover:scale-90"
      />
    ),
  },
];
